<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="adminQuery.username" placeholder="用户名" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
      ...
      <el-button v-waves class="filter-item" type="primary" icon="el-icon-plus" @click="handleAddAdmin">
        新增管理员
      </el-button>
    </div>
    ...
    <pagination v-show="total>0" :total="total" :page.sync="adminQuery.page" :limit.sync="adminQuery.limit" @pagination="getAdmins" />

    <el-dialog title="管理权限" :visible.sync="permissionDialogVisible">
      <el-form ref="permissionTemp" :model="permissionTemp" label-position="left" label-width="70px" style="width: 400px; margin-left:50px;">
        <el-form-item label="用户名" prop="username">
          <span>{{ permissionTemp.username }}</span>
        </el-form-item>
        <el-form-item label="权限列表" prop="username">
          <el-checkbox v-model="checkAllPermission" :indeterminate="isIndeterminate" @change="handleCheckAllChange">全选</el-checkbox>
          <div style="margin: 15px 0;" />
          <el-checkbox-group v-model="permissionTemp.checkedPermissions" @change="handleCheckedPermissionChange">
    ...
    <el-dialog
      title="提示"
      :visible.sync="revokeAdminDialogVisible"
      width="30%"
    >
    ...
    <el-dialog title="新增管理员" :rule="addAdminRules" :visible.sync="grantAdminDialogVisible">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="grantAdminDialogVisible = false">
          取消
        </el-button>
...
import { fetchAdminList, updatePermission, grantAdmin, revokeAdmin, fetchAdminPermission, fetchPermissionType } from '@/api/permission'
  ...
  methods: {
    ...
    handleGrantPermission(row) {
      ...
      fetchAdminPermission(row.username).then(response => {
        ...
        for (let i = 0; i < this.userPermissions.length; i++) { // 将用户已有权限添加到checked的列表中
          this.permissionTemp.checkedPermissions.push(this.userPermissions[i].permissionId)
        }
      })
      this.$nextTick(() => {
    ...
    handleCheckAllChange(val) {
      ...
      if (val) {
        for (let i = 0; i < this.permissionTypes.length; i++) { // 将用户已有权限添加到checked的列表中
    ...
    handleCheckedPermissionChange(value) {
      ...
      this.isIndeterminate = checkedCount > 0 && checkedCount < this.permissionTypes.length
    }
  }
}
</script>
